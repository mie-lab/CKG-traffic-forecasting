...
class KgContextExecutorDCRNN(TrafficStateExecutor):
    ...
    def _build_train_loss(self):
        ...
        def func(batch, batches_seen=None):
            # y_true = batch['y']  # YZflag
            y_true = batch['y_goal']  # YZflag
    ...
    def kg_evaluate(self, test_dataloader, dict_kge):
        ...
        with torch.no_grad():
            ...
            for batch in test_dataloader:
                ...
                output = self.model.predict(batch, dict_kge=dict_kge)
                try:
                    y_true = self._scaler.inverse_transform(batch['y'][..., :self.output_dim])
                except:
                    y_true = self._scaler.inverse_transform(batch['y_goal'][..., :self.output_dim])  #YZ
    ...
    def kg_train(self, train_dataloader, eval_dataloader, dict_kge):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            losses, batches_seen = self._train_epoch(train_dataloader, epoch_idx, batches_seen, self.loss_func, dict_kge)  # kg
            t1 = time.time()
            ...
            t2 = time.time()
            val_loss = self._valid_epoch(eval_dataloader, epoch_idx, batches_seen, self.loss_func, dict_kge)  # kg
            end_time = time.time()
            ...
            if val_loss < min_val_loss:
                wait = 0
                if self.saved:
                    # remove the file of last stored epoch
                    model_file_name = self.save_model_with_epoch(epoch_idx)
                    model_file_name_past = re.sub(r'epoch\d+', f'epoch{best_epoch}', model_file_name)
                    if os.path.exists(model_file_name_past):
                        os.remove(model_file_name_past)
                    self._logger.info('Val loss decrease from {:.4f} to {:.4f}, '
                                      'saving to {}, removing {}'.format(min_val_loss, val_loss, model_file_name, model_file_name_past))
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, batches_seen=None, loss_func=None, dict_kge=None):
        """
        完成模型一个轮次的训练
        ...
        for batch in train_dataloader:
            ...
            batch.to_tensor(self.device)
            loss = loss_func(batch, batches_seen, dict_kge)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, batches_seen=None, loss_func=None, dict_kge=None):
        """
        完成模型一个轮次的评估
        ...
        with torch.no_grad():
            ...
            self._writer.add_scalar('eval loss', mean_loss, batches_seen)  # epoch_idx
            return mean_loss
