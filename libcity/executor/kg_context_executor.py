...
class KgContextExecutor(TrafficStateExecutor):
    ...
    def kg_train(self, train_dataloader, eval_dataloader, dict_kge):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            losses = self._train_epoch(train_dataloader, epoch_idx, self.loss_func, dict_kge)  # kg
            t1 = time.time()
            ...
            t2 = time.time()
            val_loss = self._valid_epoch(eval_dataloader, epoch_idx, self.loss_func, dict_kge)  # kg
            end_time = time.time()
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, loss_func=None, dict_kge=None):
        ...
        with torch.no_grad():
            ...
            self._writer.add_scalar('eval loss', mean_loss, epoch_idx)  # epoch_idx
            return mean_loss
